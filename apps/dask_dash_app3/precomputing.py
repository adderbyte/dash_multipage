import pathlib
import pandas as pd
from apps.dask_dash_app3.ldacomplaints import lda_analysis
...
FILENAME = "trufeed.csv"
GLOBAL_DF = pd.read_csv("./assets/trufeed.csv", header=0)

"""
In order to make the graphs more useful we decided to prevent some words from being included
"""
ADDITIONAL_STOPWORDS = [
    '.', '(', ')', ' ', 'can', 'that', 'if', 'd', ',', '&', '  ', ':', '..', '    ', 'e', 'djdj', 'i̇', '?', ':)', 'turkey', 'trkiye', 'trkey', 'trkey', 'türkey', 'türkiyede', 'trkiye', '-', 'turkiye', 'tr', 'tr ', 'h', 'istanbul', 'türkey', 'türkiyede', 'u', 'b', 'azeraijan', 'istanl', 'nited', 'türki̇ye', 'united', 'turkye', 'azerbaijan', 'n', 's', 'djs', 'ud', 'duxh', 'f', 'djjd', 'ruj', 'shsh', 'azərbaycan', 'kktc', 'mcmx', 'euej',
...
def precompute_all_lda():
    ...
    failed_banks = []
    counter = 0
    bank_names = GLOBAL_DF["country"].value_counts().keys().tolist()
    results = {}
    ...
    for bank in bank_names:
        try:
            ...
            bank_df = GLOBAL_DF[GLOBAL_DF["country"] == bank]
            ...
            df_top3words = df_top3words_stacked.groupby(
                "topic_id").agg(", \n".join)
